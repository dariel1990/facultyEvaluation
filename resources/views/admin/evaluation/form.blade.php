                    ...
                    <div class="card-body">
                        ...
                        <form method="POST" action="/evaluations/submit/{{ $evaluationId }}">
                                        ...
                                        <tbody>
                                            @foreach($criterias as $index => $criteria)
                                                ...
                                                <tr>
                                                    ...
                                                    <td class="fw-bold text-center align-middle"><input class="radio_animated me-0" value="5" type="radio" name="rate{{ $question->id }}" {{ old('rate' . $question->id) == '5' ? 'checked' : '' }}></td>
                                                    <td class="fw-bold text-center align-middle"><input class="radio_animated me-0" value="4" type="radio" name="rate{{ $question->id }}" {{ old('rate' . $question->id) == '4' ? 'checked' : '' }}></td>
                                                    <td class="fw-bold text-center align-middle"><input class="radio_animated me-0" value="3" type="radio" name="rate{{ $question->id }}" {{ old('rate' . $question->id) == '3' ? 'checked' : '' }}></td>
                                                    <td class="fw-bold text-center align-middle"><input class="radio_animated me-0" value="2" type="radio" name="rate{{ $question->id }}" {{ old('rate' . $question->id) == '2' ? 'checked' : '' }}></td>
                                                    <td class="fw-bold text-center align-middle"><input class="radio_animated me-0" value="1" type="radio" name="rate{{ $question->id }}" {{ old('rate' . $question->id) == '1' ? 'checked' : '' }}></td>
                                                </tr>
                                                @endforeach
                                                <tr>
                            ...
                            <div class="row mt-3 h6 fw-bold">
                                <div class="col-12">
                                    <label for="comment">Remarks(Comments): @if (count($errors) > 0)
                                        <span class="text-danger">
                                            <strong>*</strong> Write a comment.
                                        </span>
                                    @endif</label>
                                    <textarea row="5" id="comment" name="comment" class="form-control">{{ old('comment') }}</textarea>
                                </div>
                            </div>
                            <div class="row mt-3 h6 fw-light">
                                <div class="col-12 text-end">
                                    <div class="d-grid">
                                        <button type="submit" class="btn btn-lg btn-primary">SUBMIT EVALUATION</button>
                                    </div>
                                </div>
                            </div>
                        </form>
